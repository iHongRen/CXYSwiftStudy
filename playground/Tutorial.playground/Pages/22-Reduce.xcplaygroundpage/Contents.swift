...
let multiply = sumArray.reduce(1, *)
...
func myReduce<T,U>(_ arr:[T],initialValue:U,combine:(U,T)->U)->U {
...
//5.用reduce来改写map函数
func mapUsingReduce<T,U>(_ xs:[T],f:(T)->U)->[U] {
    return xs.reduce([]){result,x in result + [f(x)]}//使用了系统API 尝试用自定义的
}


//6.用myReduce来改写map函数
func mapUsingMyReduce<T,U>(_ xs:[T],f:(T)->U)->[U] {
...
func filterUsingReduce<T>(_ xs:[T],check:(T)->Bool)->[T] {
    return xs.reduce([]) {
        result,x in
...
func filterUsingMyReduce<T>(_ xs:[T],check:(T)->Bool)->[T] {
    return myReduce(xs, initialValue: []) {
        result,x in
